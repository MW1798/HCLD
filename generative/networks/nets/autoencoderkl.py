...
class Decoder(nn.Module):
    ...
    def forward_noGN(self, x: torch.Tensor) -> torch.Tensor:
        for block in self.blocks:
            if not isinstance(block, nn.GroupNorm):
                x = block(x)
        return x
...
class AutoencoderKL(nn.Module):
    ...
    def decode(self, z: torch.Tensor, GN=True) -> torch.Tensor:
        """
        ...
        if self.use_checkpointing:
            ...
            dec = self.decoder.forward_noGN(z)
        return dec
    ...
    def decode_stage_2_outputs_noGN(self, z: torch.Tensor) -> torch.Tensor:
        image = self.decode(z,GN=False)
        return image
