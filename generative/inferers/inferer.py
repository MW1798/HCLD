...
class DiffusionInferer(Inferer):
    ...
    def sample(
        ...
        for t in progress_bar:
            ...
            if mode == "concat":
                ...
                if adain:  # use adain in each iteration
                    conditioning = AdaIN(conditioning,image)
    ...
    def reverse_sample(
        ...
        image = input_noise
        if verbose and has_tqdm:
            progress_bar = tqdm(reversed(scheduler.timesteps)[:-1])
        else:
            progress_bar = iter(reversed(scheduler.timesteps)[:-1])
        intermediates = []
        for t in progress_bar:
            # print(f't={t}')
            # 1. predict noise model_output
...
class LatentDiffusionInferer(DiffusionInferer):
    ...
    def __call__(
        ...
        if self.Adain == 'AdaIN':
            condition = AdaIN(condition,latent)
        elif self.Adain == 'AdaIN_reverse':
            latent = AdaIN(latent,condition)
        
        prediction, noisy_image = call(
            inputs=latent,
        ...
        return prediction, noisy_image, latent, condition
    ...
    def sample(
        ...
        if self.Adain:
            conditioning = AdaIN(conditioning,input_noise)
        ...
        if GN:
            ...
            decode = autoencoder_model.decode_stage_2_outputs_noGN  # do not use group normalization 
    ...
    def reverse_sample(
        ...
        assert isinstance(scheduler, DDIMScheduler), "Only DDIMScheduler is supported for reverse sampling."

        reverse_sample = super().reverse_sample
        # if isinstance(diffusion_model, SPADEDiffusionModelUNet):
        #     sample = partial(super().sample, seg=seg)
